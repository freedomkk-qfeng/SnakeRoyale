...
- `tick` 对普通运行时是可选字段。SDK 客户端会回显最近一次收到的 `state.tick`，供 benchmark 在起跑阶段过滤 reset 之前的陈旧 move；如果要参与 benchmark，则需要发送该字段。
...
返回浏览器里的 benchmark 回放查看页。

- 上传 `replay.jsonl` 后，可以逐 tick 拖动和回放正式 benchmark 计时窗口。
- 如果同时上传 `summary.json`，页面会额外展示按算法聚合的赢家和统计摘要。
- 这两个文件都应当来自 benchmark runner，并带有相同的 `benchmark_run_id`。
- 如果 `summary.json` 与当前回放的 `benchmark_run_id` 或核心 benchmark 元数据不一致，页面会拒绝叠加。
...
我们提供了一套 Python client SDK，并内置两个算法客户端：

- `client.py` — BFS 寻路客户端
- `random_client.py` — 随机安全移动客户端
...
- **[📄 查看默认 BFS 入口源码 (client.py)](/api/client-source)** — 在浏览器中查看 BFS 入口实现
- **[⬇ 下载 client SDK 压缩包](/download/client-sdk.zip)** — 下载 `client.py`、`random_client.py`、`sdk.py` 等文件
...
python random_client.py --server http://<host>:15000 --name "你的随机 bot"
```
